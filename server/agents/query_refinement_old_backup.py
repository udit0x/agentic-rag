"""Query Refinement Agent for generating typed retrieval-aware queries."""
import logging
...
""")
    ...
    async def generate_related_questions(
        ...
        """
        Generate related questions for the given query with smart caching.
        ...
        Args:
            ...
            num_questions: Number of questions to generate (default: 5, HYBRID mode: 3)
        ...
        if session_id and not force_regenerate:
            ...
            if cached_result:
                ...
                logger.info("Using cached result (similarity: %.2f)", similarity)
                
                # Limit cached queries to requested number
                limited_queries = cache_entry.refined_queries[:num_questions]
                
                # Return cached result with metadata
                return QueryRefinement(
                    original_query=query,
                    refined_queries=limited_queries,
        ...
        try:
            llm = get_llm()
            if not llm:
                return self._fallback_refinement(query, num_questions)
            
            # Determine mode description
            mode_desc = "HYBRID (focused retrieval with conversation context)" if num_questions == 3 else "RAG (comprehensive retrieval)"
            
            # Build extra question placeholders for prompt
            extra_questions = ""
            if num_questions > 2:
                extra_questions = ",\n    ".join([f'"Question {i}"' for i in range(3, num_questions + 1)])
                if extra_questions:
                    extra_questions = ",\n    " + extra_questions
            ...
            if "refined_queries" in result and len(result["refined_queries"]) > num_questions:
                result["refined_queries"] = result["refined_queries"][:num_questions]
            ...
            try:
                ...
                if self._is_api_error(str(cleanup_error)):
                    logger.warning("API error in cleanup - skipping question generation")
            ...
            fallback_result = self._fallback_refinement(query, num_questions)
    ...
    def _fallback_refinement(self, query: str, num_questions: int = 5) -> QueryRefinement:
        """
        ...
        Args:
            query: The original user query
            num_questions: Number of questions to generate (default: 5, HYBRID: 3)
        """
        ...
        limited_questions = fallback_questions[:num_questions]
        
        return QueryRefinement(
            original_query=query,
            refined_queries=limited_questions,
            query_category="factual",
            refinement_reasoning=f"Generated {num_questions} contextually appropriate questions for {topic} based on query pattern analysis (LLM parsing error fallback)",
            was_cached=False,
            cache_similarity=0.0,
            cost_savings={
